...
- Google Cloud boundary planning recorded:
  ...
  - Cloud Run deployment runbook lives at `rcs-registration/cloud-run/revolut-webhook/DEPLOYMENT_PREP.md`; endpoint proof is still pending and the Revolut webhook URL has not been changed;
  ...
  - Firestore Native mode remains the dedupe/event store choice, but no deployed webhook proof has written to it yet;
  - first deployed mode must be record-only;
  - creating service account keys, additional IAM grants, changing Revolut webhook URL, automatic Billing writes, and strict payment gating remain explicitly forbidden until approved.
