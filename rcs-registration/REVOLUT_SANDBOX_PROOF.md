...
- Initial refund response summary exposed `type: "REFUND"` and `state: "PROCESSING"` from the proof tool's then-current summary. The authoritative later direct retrieval of the refund order returned lowercase `type: "refund"` and `state: "completed"`; endpoint enrichment code should normalise casing and treat lowercase `refund` as the observed sandbox retrieval shape.
